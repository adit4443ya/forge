...
export const ESTIMATES = [
  ...
  {
    id: "e15", kind: "compute", d: 3,
    q: "How many rows can you scan per second from an in-memory columnar table, and does that make an index worth it?",
    anchors: ["A core streams sequentially at several GB/s", "A column of 4-byte integers is 4 bytes per row"],
    work: [
      "Sequential scan of one 4-byte column at ~5 GB/s is ~1.2e9 rows/s per core.",
      "A billion-row table therefore scans in about a second per column touched, per core; across 8 cores, ~150 ms.",
      "An index lookup is a handful of random accesses at ~100 ns each — call it a microsecond — but returns one row.",
    ],
    answer: "Around a billion rows per second per core scanning one column; an index is worth it only below roughly 1% selectivity.",
    range: "5e8 – 2e9 rows/s per core",
    trust: "How many columns the query touches. Columnar scans cost per column, so a query reading ten columns is ten times this, and that single factor usually decides the answer.",
    matters: "It sets the crossover: below about 1% selectivity the index wins, above it the scan is faster because it is sequential and the index is not. Knowing the crossover is what stops you adding an index that makes things slower.",
  },
  {
    id: "e16", kind: "market", d: 3,
    q: "How many orders can one gateway send per second before you are the bottleneck rather than the exchange?",
    anchors: ["An order message is a few hundred bytes", "A syscall is on the order of a microsecond; kernel bypass is far less"],
    work: [
      "Through the kernel: one sendmsg per order at ~1–2 µs of CPU means ~5e5 to 1e6 orders/s per core, ignoring everything else you must also do.",
      "With kernel bypass the per-message cost drops to hundreds of nanoseconds, so a few million per second becomes possible.",
      "But exchanges rate-limit — typically thousands to tens of thousands of messages a second per session — so the venue binds long before the box does.",
    ],
    answer: "Hundreds of thousands per second through the kernel, millions with bypass — and the exchange's limit is one to two orders of magnitude below either.",
    range: "1e5 – 1e6 through the kernel per core",
    trust: "That syscall cost is the dominant term. Once you add risk checks, book updates and strategy logic, the syscall may be a minority of the budget, which changes what to optimise.",
    matters: "It tells you where to spend effort. If the venue caps you at ten thousand messages a second, the throughput of your gateway is irrelevant and the only thing that matters is the LATENCY of each one — a completely different optimisation.",
  },
  {
    id: "e17", kind: "systems", d: 4,
    q: "Your service has a p99 of 10 ms. A request fans out to 20 services in parallel. What is the p99 of the whole request?",
    anchors: ["The slowest branch determines a parallel fan-out", "P(all fast) = product of per-service probabilities"],
    work: [
      "If each service independently exceeds 10 ms with probability 1%, the chance ALL twenty stay under is 0.99^20 ≈ 0.82.",
      "So about 18% of requests hit at least one slow branch — the overall p99 is no longer 10 ms; roughly the p99.95 of a single service is what now shapes it.",
      "To keep the whole request at p99 = 10 ms, each service needs to be under 10 ms about 99.95% of the time.",
    ],
    answer: "Far worse than 10 ms. Fanning out to 20 turns a per-service p99 into roughly a per-request p82.",
    range: "the whole-request p99 lands near each service's p99.95",
    trust: "Independence. Slow branches are usually correlated — a shared dependency, a garbage collection pause, a hot shard — which can make it either better or considerably worse, and correlated tails are the common real case.",
    matters: "It is the arithmetic behind tail-tolerant design: hedged requests, tied requests, and per-branch timeouts exist because the alternative is a tail that grows with fan-out. It also explains why percentiles do not average across shards.",
  },
  {
    id: "e18", kind: "classic", d: 3,
    q: "How much data does a self-driving car generate per hour, and can you send it anywhere?",
    anchors: ["Cameras at a few megapixels and tens of frames per second", "Lidar returning hundreds of thousands of points per second"],
    work: [
      "Eight cameras at 2 MP, 3 bytes per pixel, 30 fps: 8 × 2e6 × 3 × 30 ≈ 1.4e9 B/s raw, so ~1.4 GB/s.",
      "Lidar at ~1e6 points/s × ~16 bytes is small by comparison, ~16 MB/s.",
      "Over an hour: ~5 TB raw, perhaps 500 GB after realistic video compression.",
    ],
    answer: "Terabytes per hour raw, hundreds of gigabytes compressed.",
    range: "1–10 TB/h raw",
    trust: "The compression ratio, which varies enormously with scene content and how much fidelity the perception stack needs. Compressing away the thing you were trying to detect is the failure mode.",
    matters: "It settles the architecture: you cannot stream this off the vehicle, so inference must be on board and only interesting segments are uploaded. The estimate is what makes 'do it at the edge' an argument rather than an opinion.",
  },
  {
    id: "e19", kind: "compute", d: 3,
    q: "How long to sort a terabyte of 8-byte integers, and what does that tell you about the algorithm?",
    anchors: ["A core sorts at roughly 1e8 keys/s in memory", "1 TB of 8-byte keys is 1.25e11 keys"],
    work: [
      "1.25e11 keys ÷ 1e8 per second ≈ 1250 core-seconds, so ~20 minutes on one core.",
      "Across 32 cores, if it parallelises well: under a minute of CPU.",
      "But 1 TB does not fit in memory, so it is an external sort: read, sort runs, write, merge — at least two full passes over disk. At 3 GB/s that is ~11 minutes of I/O alone.",
    ],
    answer: "Bounded by I/O, not comparison: roughly ten to thirty minutes, dominated by the passes over storage.",
    range: "10 – 60 min",
    trust: "Whether it fits in memory. That single question changes the algorithm completely, and 'is the working set larger than RAM' is the first thing to establish rather than the last.",
    matters: "It is why external sorting minimises the number of passes rather than the number of comparisons, and why a radix sort — no comparisons at all — is the usual choice for fixed-width integer keys at this scale.",
  },
];
